    ...
    <%--AJAX--%>
        <script>
            $(document).ready(function() {
                var currentUserNum = ${sessionScope.userNum}; // 서버 측에서 세션에 저장된 사용자 번호를 가져옴
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        var url = response.isBookRead ? '/pages/user-myStatRead' : '/pages/user-myStatUnread';

                        $('#elements').load(url, function(response, status, xhr) {
                            if (status == "error") {
                                console.error("콘텐츠 로드 중 오류 발생:", xhr.status, xhr.statusText);
                            }
                        });
                    },
                    error: function(xhr, status, error) {
                        console.error("독서 상태 확인 중 오류 발생:", error);
                        $('#elements').html("<p>데이터를 불러오는 데 실패했습니다. 새로고침을 해주세요.</p>");
                    }
                });
            });
        </script>
